...
const qrForm = document.querySelector("#qrcode-display form");
const messageForm = document.querySelector("#message-display form");
  ...
  qrForm.addEventListener("submit", () => {
    setTimeout(() => {
      ...
      if (spinnerModal.open == True) {
        setTimeout(() => {
          spinnerModal.close();
          alert("QR Code display operation timed out.")
        }, 61.0 * 1000);
      }
    }, 500);
  });
  ...
  messageForm.addEventListener("submit", () => {
    setTimeout(() => {
      ...
      if (spinnerModal.open == True) {
        setTimeout(() => {
          spinnerModal.close();
          alert("Message display operation timed out.")
        }, 61.0 * 1000);
      }
    }, 500);
  });
...
// Send tile action via the routing URL to the server
